...
import { FaBorderNone, FaRegGem, FaGem, FaBars } from "react-icons/fa";
...
function Navbar(props) {
  ...
  const formatAddress = () => {
    return `${String(user.wallet).substring(0, 6)}...${String(
      user.wallet
    ).substring(38)}`;
  };
  ...
  return (
    <div>
      <nav
        className="navbar"
        style={{
          backgroundColor: scrolled ? "rgba(25, 27, 31, 0.9)" : "#191B1F",
        }}
      >
        <div className="logo-container">
          <FaGem color="#FF007A" fontSize="2.8em" />
        ...
        <div className="menu-btn" onClick={() => setShow(!show)}>
          <FaBars color="white" fontSize="2em" />
        </div>
      </nav>
      {show && (
